...
# NCAAB team list for dropdowns (top 25 + major conferences)
NCAAB_TEAMS = [
    "Alabama", "Arizona", "Arkansas", "Auburn", "Baylor", "Butler", "BYU", "Cincinnati", "Clemson", "Colorado",
    "Connecticut", "Creighton", "Duke", "Florida", "Florida St", "Gonzaga", "Houston", "Illinois", "Indiana", "Iowa",
    "Iowa St", "Kansas", "Kansas St", "Kentucky", "Louisville", "Marquette", "Maryland", "Memphis", "Miami FL", "Michigan",
    "Michigan St", "Missouri", "NC State", "North Carolina", "Notre Dame", "Ohio St", "Oklahoma", "Oregon", "Purdue", "San Diego St",
    "Seton Hall", "St. John's", "Syracuse", "TCU", "Tennessee", "Texas", "Texas A&M", "Texas Tech", "UCLA", "USC",
    "Villanova", "Virginia", "Wake Forest", "Washington", "West Virginia", "Wisconsin", "Xavier"
]
...
def manual_input_form():
    """Serve HTML form for manual NCAAB gameline input"""
    html_content = f"""
    <html>
    <head>
    ...
    <body>
        ...
        <form action="/ncaab/gamelines/manual" method="post">
            ...
            <button type="submit">Submit NCAAB Gameline</button>
        </form>
    </body>
    </html>
    """
    ...
    <body>
        ...
        <script>
            document.getElementById('teamForm').onsubmit = async function(e) {{
                ...
                if (team && year) {{
                    try {{
                        const response = await fetch(`/ncaab/${{team}}/${{year}}`);
...
def player_select_form():
    ...
    <body>
        <h2>NCAAB Player Statistics (Coming Soon)</h2>
...
@app.get("/ncaab/coach-stats", response_class=HTMLResponse)
def coach_select_form():
    """Serve HTML form for coach stats (placeholder)"""
    html_content = """
    <html>
    <head>
        <title>NCAAB Coach Stats</title>
    </head>
    <body>
        <h2>NCAAB Coach Statistics (Coming Soon)</h2>
